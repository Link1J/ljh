...
struct not_move
{
    not_move()                           = default;
    ~not_move()                          = default;
    not_move(not_move const&)            = default;
    not_move(not_move&&)                 = delete;
    not_move& operator=(not_move const&) = default;
...
struct not_copy
{
    ...
    not_copy& operator=(not_copy const&) = delete;
...
{
    ljh::unexpected<char const*> test{"Test"};
    // is a compile test
}

TEST_CASE("expected<trivial_data_type, std::string>::emplace", "[test_17][expected][emplace]")
{
    ljh::expected<trivial_data_type, std::string> test;
    REQUIRE(test.has_value());
