...
TEST_CASE("nested generator", "[test_20][generator]")
{
    ...
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM user WHERE id = 4;");
    it++;
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM user WHERE id = 7;");
    it++;
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM user WHERE id = 9;");
    it++;
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM user WHERE id = 10;");
    it++;
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM order WHERE id = 11;");
    it++;
    REQUIRE(it != end);
    CHECK(*it == "DELETE FROM order WHERE id = 19;");
    it++;
    REQUIRE(it == end);
}
...
{
    explicit unknown(std::string item)
        : std::invalid_argument(std::string{"Unknown "} + item)
    {}
};
