...
#define COMPILER_VALUE_INT(n)                                                                                                                                  \
    #n[0] == '_' ? 0 : (#n[5] - '0') + (#n[4] - '0') * 10 + (#n[3] - '0') * 100 + (#n[2] - '0') * 1000 + (#n[1] - '0') * 10000 + (#n[0] - '0') * 100000
...
public:
    ...
    constexpr long std() const
    {
        return std_;
    }
    constexpr long data() const
    {
        return data_;
    }
    void data(long x)
...
static /*constexpr*/ std::pair<char const*, char const*> compiler[] = {
    COMPILER_SPECIFIC_ENTRY(__cplusplus), //< not compiler specific, but useful :)
    COMPILER_SPECIFIC_ENTRY(__clang_major__), COMPILER_SPECIFIC_ENTRY(__clang_minor__), COMPILER_SPECIFIC_ENTRY(__clang_patchlevel__),
    COMPILER_SPECIFIC_ENTRY(__GNUG__),        COMPILER_SPECIFIC_ENTRY(__GNUC_MINOR__),  COMPILER_SPECIFIC_ENTRY(__GNUC_PATCHLEVEL__),
    COMPILER_SPECIFIC_ENTRY(__GNUG__),        COMPILER_SPECIFIC_ENTRY(__GNUC_MINOR__),  COMPILER_SPECIFIC_ENTRY(__GNUC_PATCHLEVEL__),
    COMPILER_SPECIFIC_ENTRY(_MSC_FULL_VER),   COMPILER_SPECIFIC_ENTRY(_MSVC_LANG),
...
static constexpr CompilerFeature cxx26_core[] = {
    ...
    COMPILER_FEATURE_ENTRY(202403L, __cpp_deleted_function), COMPILER_FEATURE_ENTRY(202406L, __cpp_fold_expressions),
    COMPILER_FEATURE_ENTRY(202311L, __cpp_pack_indexing),    COMPILER_FEATURE_ENTRY(202306L, __cpp_placeholder_variables),
    COMPILER_FEATURE_ENTRY(202306L, __cpp_static_assert),    COMPILER_FEATURE_ENTRY(202411L, __cpp_structured_bindings),
...
static constexpr CompilerFeature attributes[] = {
    COMPILER_ATTRIBUTE(202207L, assume),       COMPILER_ATTRIBUTE(200809L, carries_dependency), COMPILER_ATTRIBUTE(201309L, deprecated),
    COMPILER_ATTRIBUTE(201603L, fallthrough),  COMPILER_ATTRIBUTE(202403L, indeterminate),      COMPILER_ATTRIBUTE(201803L, likely),
    COMPILER_ATTRIBUTE(201603L, maybe_unused), COMPILER_ATTRIBUTE(201803L, no_unique_address),  COMPILER_ATTRIBUTE(201907L, nodiscard),
...
inline void print_compiler_feature(CompilerFeature const& x)
{
    if (not((print.supported_features and x.maybe()) or (print.unsupported_features and not x.maybe())))
        return;
    auto print_year_month = [](long n) { return std::printf("%ld%s%02ld", n / 100, print.separate_year_month ? "-" : "", n % 100); };
