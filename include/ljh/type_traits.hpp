...
namespace ljh
{
    LJH_MODULE_MAIN_EXPORT template<typename T, bool = std::is_enum<T>::value>
    ...
    LJH_MODULE_MAIN_EXPORT template<typename T, template<typename...> class U>
    using is_instance = _is_instance_impl<std::decay_t<T>, U>;

    LJH_MODULE_MAIN_EXPORT template<typename, typename = void>
    struct is_type_complete : std::false_type
    {};

    LJH_MODULE_MAIN_EXPORT template<typename...>
    using void_t = void;
    ...
    LJH_MODULE_MAIN_EXPORT template<typename T, typename = void>
    ...
    LJH_MODULE_MAIN_EXPORT template<typename T, typename U>
    struct make_like
    {
        using type = U;
    };
