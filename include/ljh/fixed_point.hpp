...
namespace ljh
{
    ...
    LJH_MODULE_MATH_EXPORT template<std::size_t _storage_bits_size, std::size_t _factional_bits_size, bool _signed>
    struct fixed_point
    {
    private:
    ...
    public:
        ...
        friend constexpr fixed_point operator+(fixed_point const& lhs, fixed_point const& rhs)
        {
        ...
        constexpr bool operator==(fixed_point const&) const noexcept = default;
#else
        constexpr bool operator==(const fixed_point& rhs) const noexcept
        {
            return _data == rhs._data;
        }
        constexpr bool operator<(fixed_point const& rhs) const noexcept
        {
            return _data < rhs._data;
        }
        constexpr bool operator!=(fixed_point const& rhs) const noexcept
