...
namespace ljh
{
	...
	[[nodiscard]] auto hsl_to_rgb(double h, double s, double l) -> std::tuple<uint8_t, uint8_t, uint8_t>
	{
	...
	[[nodiscard]] auto rgb_to_hsl(uint8_t r, uint8_t g, uint8_t b) -> std::tuple<double, double, double>
	{
		double dr = r / 255.;
		double dg = g / 255.;
		double db = b / 255.;
