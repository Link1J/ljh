...
namespace ljh
{
	...
	{
		return []<typename... Args>(Args&&... args) requires std::constructible_from<T, Args...>
		{
...
namespace ljh::ranges::views
{
	...
	{
		return std::ranges::views::transform(std::forward<R>(r), [pred = std::forward<F>(pred)]<typename T>(T&& t) 
		{
			return std::apply(pred, std::forward<T>(t));
		});
	};
