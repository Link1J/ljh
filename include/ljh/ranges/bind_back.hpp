...
namespace ljh::__
{
    ...
    template<typename F, typename... A>
    struct can_bind_back
        : std::bool_constant<std::is_constructible<typename std::decay<F>::type, F>::value && std::is_move_constructible<typename std::decay<F>::type>::value &&
                             (std::is_constructible<typename std::decay<A>::type, A>::value && ...) &&
                             (std::is_move_constructible<typename std::decay<A>::type>::value && ...)>
    {};
} // namespace ljh::__

namespace ljh
{
    LJH_MODULE_MAIN_EXPORT template<typename F, typename... A, typename = typename std::enable_if<__::can_bind_back<F, A...>::value>::type>
...
namespace ljh
{
    LJH_MODULE_MAIN_EXPORT using std::bind_back;
}
#endif
