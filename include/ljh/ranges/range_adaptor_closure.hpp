...
namespace ljh::ranges
{
    ...
    struct pipeline : range_adaptor_closure<pipeline<Left, Right>>
    {
        ...
        constexpr explicit pipeline(T1&& f1, T2&& f2) noexcept(std::is_nothrow_convertible_v<T1, Left> && std::is_nothrow_convertible_v<T2, Right>)
            : l(std::forward<T1>(f1))
            , r(std::forward<T2>(f2))
        {}

        template<typename T>
        [[nodiscard]] constexpr auto operator()(T&& val) noexcept(noexcept(r(l(std::forward<T>(val)))))
        ...
        template<typename T>
        [[nodiscard]] constexpr auto operator()(T&& val) const noexcept(noexcept(r(l(std::forward<T>(val)))))
            requires requires { r(l(std::forward<T>(val))); }
        {
            return r(l(std::forward<T>(val)));
        }
    ...
    struct range_adaptor_closure
    {
        ...
        [[nodiscard]] friend constexpr auto operator|(C1&& c1, C2&& c2) noexcept(std::is_nothrow_constructible_v<std::decay_t<C1>, C1> &&
                                                                                 std::is_nothrow_constructible_v<std::decay_t<C2>, C2>)
        {
